...
const create = (params, callback) => {
  ...
  const query = `
    INSERT INTO Usuario (nombre, apellido, claveAcceso, correo,
      telefono, fotografia, genero, fechaNacimiento, direccion,
      rol, estado, paisID)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
  `;
...
const getProfile = (params, callback) => {
  ...
  const query = `
    SELECT u.nombre name, apellido lastname, correo email,
      telefono phone, fotografia photo, genero gender,
...
const update = (params, callback) => {
  ...
  let query = `
    UPDATE Usuario SET  nombre =  ?, apellido = ?,
      correo = ?, telefono = ?, genero = ?, fechaNacimiento = ?,
      direccion = ?, paisID = ?
  `;
