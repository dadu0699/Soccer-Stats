...
SELECT Estadio.estadioID AS id_stadium, Estadio.nombre AS stadium, Estadio.capacidad AS capacity,
Estadio.foto AS photo, Estadio.paisID AS id_country, Pais.nombre AS country FROM Estadio
INNER JOIN Pais ON Pais.paisID = Estadio.paisID
WHERE Estadio.capacidad <= 25000;

-- HISTORICOS DE PARTIDOS DE X PARTIDO
SELECT 
...
WHERE Partido.equipoVisitaID = 1 OR Partido.equipoLocalID = 1; -- Enviar el mismo id de equipo en local y visitante

-- EQUIPOS DONDE HA ESTADO O DIRIGIDO X JUGADOR O DIRECTOR
